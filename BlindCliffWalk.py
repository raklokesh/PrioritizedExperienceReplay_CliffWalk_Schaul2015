...
def find_transition():
    if agent == 'R':
        ...
        return None,transition_list[np.argmin(transition_priorities)]
    elif agent == 'TD':
...
max_updates = [10000,10000,10000,10000] # Can tune this to reduce run time for different state lengths
Episodes = [1, 1, 1, 1] # Number of episodes for each agent
for agent_no,agent in enumerate(Agents):
    MSE = np.zeros(1000000)
...
MSE_plot.set_ylabel('MSE (average 10 runs)')
MSE_plot.set_xlabel('Updates')
MSE_plot.set_xlim((0,10000))
MSE_plot.legend()
